...
const getOnboardPatientSchema = (optionalEmail: boolean = false) =>
  z.object({
...
export type AddPatientFormData = z.infer<
  ReturnType<typeof getOnboardPatientSchema>
>;

interface UseOnboardPatientFormProps {
...
export function useOnboardPatientForm({
  onSubmit,
  isPending = false,
  optionalEmail = false,
  defaultValues,
}: UseOnboardPatientFormProps) {
  const form = useForm<AddPatientFormData>({
    resolver: zodResolver(getOnboardPatientSchema(optionalEmail)),
    mode: 'onChange',
    defaultValues: {
      fullName: '',
